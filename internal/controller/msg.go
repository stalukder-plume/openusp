...
func (c *Cntlr) processRxUspMsg(epId string, mData *uspMsgData) error {
	...
	case usp_msg.Header_ERROR:
		if mData.err != nil {
			...
			c.cacheSetError(epId, mData.id, cErr)
		} else {
	...
	case usp_msg.Header_GET_RESP:
		...
		if len(mData.params) > 0 {
			if err := c.dbWriteParams(epId, mData.params); err != nil {
				return err
			}
		}
	case usp_msg.Header_GET_INSTANCES_RESP:
		...
		if len(mData.instances) > 0 {
			if err := c.dbWriteInstances(epId, mData.instances); err != nil {
				return err
			}
		}
	case usp_msg.Header_ADD_RESP:
		...
		c.cacheSetInstance(epId, mData.id, cInst)
		if len(mData.instances) > 0 {
			if err := c.dbWriteInstances(epId, mData.instances); err != nil {
				return err
			}
	...
	case usp_msg.Header_SET_RESP:
		...
		c.cacheSetParamSetResult(epId, mData.id, cSetResult)
		if len(mData.instances) > 0 {
			if err := c.dbWriteInstances(epId, mData.instances); err != nil {
				return err
			}
	...
	case usp_msg.Header_GET_SUPPORTED_DM_RESP:
		if len(mData.dms) > 0 {
			if err := c.dbWriteDatamodels(mData.dms); err != nil {
				return err
			}
		}
	case usp_msg.Header_DELETE_RESP:
		...
		if len(mData.paths) > 0 {
			if err := c.dbDeleteInstances(epId, mData.paths); err != nil {
				return err
			}
			if err := c.dbDeleteParams(epId, mData.paths); err != nil {
				return err
			}
		}
